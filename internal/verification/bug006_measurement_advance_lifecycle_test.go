...
// internal/domain/measurements.go
// internal/app/measurement_service.go
// internal/httpapi/measurement_handler.go
func TestBug006MeasurementAdvanceLifecycle(t *testing.T) {
	svc := newService()
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	createdRec := postJSON(router, "/api/v1/measurements", `{"name":"sample-6","owner":"operator"}`)
	...
	advancedRec := postJSON(router, "/api/v1/measurements/"+string(created.ID)+"/advance", `{"status":"active","actor":"reviewer"}`)
	if advancedRec.Code != http.StatusOK {
		t.Fatalf("advance status=%d body=%s", advancedRec.Code, advancedRec.Body.String())
	}
	var advanced domain.Measurement
...
func TestBug006InvalidTransition(t *testing.T) {
	svc := newService()
	created, err := svc.CreateMeasurement(domain.Measurement{Name: "sample-6", Owner: "operator"})
	if err != nil {
		t.Fatal(err)
	}
	_, err = svc.AdvanceMeasurement(created.ID, domain.StatusCompleted, "reviewer")
