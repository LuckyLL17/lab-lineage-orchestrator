...
// internal/domain/sample_batches.go
// internal/app/sampleBatch_service.go
// internal/httpapi/sampleBatch_handler.go
func TestBug002SamplebatchAdvanceLifecycle(t *testing.T) {
	svc := newService()
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	createdRec := postJSON(router, "/api/v1/sample-batches", `{"name":"sample-2","owner":"operator"}`)
	if createdRec.Code != http.StatusCreated {
		t.Fatalf("create status=%d body=%s", createdRec.Code, createdRec.Body.String())
	}
	var created domain.SampleBatch
	decodeJSON(t, createdRec, &created)
	advancedRec := postJSON(router, "/api/v1/sample-batches/"+string(created.ID)+"/advance", `{"status":"active","actor":"reviewer"}`)
	if advancedRec.Code != http.StatusOK {
		t.Fatalf("advance status=%d body=%s", advancedRec.Code, advancedRec.Body.String())
	}
	var advanced domain.SampleBatch
...
func TestBug002InvalidTransition(t *testing.T) {
	svc := newService()
	created, err := svc.CreateSampleBatch(domain.SampleBatch{Name: "sample-2", Owner: "operator"})
	if err != nil {
		t.Fatal(err)
	}
	_, err = svc.AdvanceSampleBatch(created.ID, domain.StatusCompleted, "reviewer")
