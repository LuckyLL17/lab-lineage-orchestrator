...
func TestBug028SearchLimitPropagation(t *testing.T) {
	svc := newService()
	svc.Ingest(app.IngestEnvelope{Kind: "search", Actor: "operator", Payload: "search-a"})
	svc.Ingest(app.IngestEnvelope{Kind: "search", Actor: "operator", Payload: "search-b"})
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/search?q=search&limit=1", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status=%d", rec.Code)
	}
	var results []domain.EntityView
...
func TestBug028HealthySearchNoMatch(t *testing.T) {
	svc := newService()
