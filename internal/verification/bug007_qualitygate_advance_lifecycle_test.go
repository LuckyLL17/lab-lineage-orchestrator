...
// internal/domain/quality_gates.go
// internal/app/qualityGate_service.go
// internal/httpapi/qualityGate_handler.go
func TestBug007QualitygateAdvanceLifecycle(t *testing.T) {
	svc := newService()
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	createdRec := postJSON(router, "/api/v1/quality-gates", `{"name":"sample-7","owner":"operator"}`)
	if createdRec.Code != http.StatusCreated {
		t.Fatalf("create status=%d body=%s", createdRec.Code, createdRec.Body.String())
	}
	var created domain.QualityGate
	decodeJSON(t, createdRec, &created)
	advancedRec := postJSON(router, "/api/v1/quality-gates/"+string(created.ID)+"/advance", `{"status":"active","actor":"reviewer"}`)
	if advancedRec.Code != http.StatusOK {
		t.Fatalf("advance status=%d body=%s", advancedRec.Code, advancedRec.Body.String())
	}
	var advanced domain.QualityGate
...
func TestBug007InvalidTransition(t *testing.T) {
	svc := newService()
	created, err := svc.CreateQualityGate(domain.QualityGate{Name: "sample-7", Owner: "operator"})
	if err != nil {
		t.Fatal(err)
	}
	_, err = svc.AdvanceQualityGate(created.ID, domain.StatusCompleted, "reviewer")
