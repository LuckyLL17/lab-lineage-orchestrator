...
// internal/domain/experiment_runs.go
// internal/app/experimentRun_service.go
// internal/httpapi/experimentRun_handler.go
func TestBug004ExperimentrunAdvanceLifecycle(t *testing.T) {
	svc := newService()
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	createdRec := postJSON(router, "/api/v1/experiment-runs", `{"name":"sample-4","owner":"operator"}`)
	if createdRec.Code != http.StatusCreated {
		t.Fatalf("create status=%d body=%s", createdRec.Code, createdRec.Body.String())
	}
	var created domain.ExperimentRun
	decodeJSON(t, createdRec, &created)
	advancedRec := postJSON(router, "/api/v1/experiment-runs/"+string(created.ID)+"/advance", `{"status":"active","actor":"reviewer"}`)
	if advancedRec.Code != http.StatusOK {
		t.Fatalf("advance status=%d body=%s", advancedRec.Code, advancedRec.Body.String())
	}
	var advanced domain.ExperimentRun
...
func TestBug004InvalidTransition(t *testing.T) {
	svc := newService()
	created, err := svc.CreateExperimentRun(domain.ExperimentRun{Name: "sample-4", Owner: "operator"})
	if err != nil {
		t.Fatal(err)
	}
	_, err = svc.AdvanceExperimentRun(created.ID, domain.StatusCompleted, "reviewer")
