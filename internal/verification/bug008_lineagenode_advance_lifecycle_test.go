...
// internal/domain/lineage_nodes.go
// internal/app/lineageNode_service.go
// internal/httpapi/lineageNode_handler.go
func TestBug008LineagenodeAdvanceLifecycle(t *testing.T) {
	svc := newService()
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	createdRec := postJSON(router, "/api/v1/lineage-nodes", `{"name":"sample-8","owner":"operator"}`)
	if createdRec.Code != http.StatusCreated {
		t.Fatalf("create status=%d body=%s", createdRec.Code, createdRec.Body.String())
	}
	var created domain.LineageNode
	decodeJSON(t, createdRec, &created)
	advancedRec := postJSON(router, "/api/v1/lineage-nodes/"+string(created.ID)+"/advance", `{"status":"active","actor":"reviewer"}`)
	if advancedRec.Code != http.StatusOK {
		t.Fatalf("advance status=%d body=%s", advancedRec.Code, advancedRec.Body.String())
	}
	var advanced domain.LineageNode
...
func TestBug008InvalidTransition(t *testing.T) {
	svc := newService()
	created, err := svc.CreateLineageNode(domain.LineageNode{Name: "sample-8", Owner: "operator"})
	if err != nil {
		t.Fatal(err)
	}
	_, err = svc.AdvanceLineageNode(created.ID, domain.StatusCompleted, "reviewer")
