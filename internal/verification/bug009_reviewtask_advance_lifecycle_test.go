...
// internal/domain/review_tasks.go
// internal/app/reviewTask_service.go
// internal/httpapi/reviewTask_handler.go
func TestBug009ReviewtaskAdvanceLifecycle(t *testing.T) {
	svc := newService()
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	createdRec := postJSON(router, "/api/v1/review-tasks", `{"name":"sample-9","owner":"operator"}`)
	...
	advancedRec := postJSON(router, "/api/v1/review-tasks/"+string(created.ID)+"/advance", `{"status":"active","actor":"reviewer"}`)
	if advancedRec.Code != http.StatusOK {
		t.Fatalf("advance status=%d body=%s", advancedRec.Code, advancedRec.Body.String())
	}
	var advanced domain.ReviewTask
...
func TestBug009InvalidTransition(t *testing.T) {
	svc := newService()
	created, err := svc.CreateReviewTask(domain.ReviewTask{Name: "sample-9", Owner: "operator"})
	if err != nil {
		t.Fatal(err)
	}
	_, err = svc.AdvanceReviewTask(created.ID, domain.StatusCompleted, "reviewer")
