...
func TestBug016SearchPayloadFlow(t *testing.T) {
	svc := newService()
	if _, err := svc.Ingest(app.IngestEnvelope{Kind: "measurement", Actor: "operator", Payload: "unique-payload-16"}); err != nil {
		t.Fatal(err)
	}
	results := svc.Search("unique-payload-16", 10)
	if len(results) != 1 {
