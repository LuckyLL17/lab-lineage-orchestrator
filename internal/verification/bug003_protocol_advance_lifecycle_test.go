...
// internal/domain/protocols.go
// internal/app/protocol_service.go
// internal/httpapi/protocol_handler.go
func TestBug003ProtocolAdvanceLifecycle(t *testing.T) {
	svc := newService()
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	createdRec := postJSON(router, "/api/v1/protocols", `{"name":"sample-3","owner":"operator"}`)
	...
	advancedRec := postJSON(router, "/api/v1/protocols/"+string(created.ID)+"/advance", `{"status":"active","actor":"reviewer"}`)
	if advancedRec.Code != http.StatusOK {
		t.Fatalf("advance status=%d body=%s", advancedRec.Code, advancedRec.Body.String())
	}
	var advanced domain.Protocol
...
func TestBug003InvalidTransition(t *testing.T) {
	svc := newService()
	created, err := svc.CreateProtocol(domain.Protocol{Name: "sample-3", Owner: "operator"})
	if err != nil {
		t.Fatal(err)
	}
	_, err = svc.AdvanceProtocol(created.ID, domain.StatusCompleted, "reviewer")
