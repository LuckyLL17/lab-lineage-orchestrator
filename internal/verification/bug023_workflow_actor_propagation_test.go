...
func TestBug023WorkflowActorPropagation(t *testing.T) {
	svc := newService()
	event, err := svc.RunWorkflow(app.WorkflowRequest{Name: "lineage", Actor: "reviewer-23", Inputs: []domain.ID{"input-a"}, Mode: "execute"})
	if err != nil {
		t.Fatal(err)
	}
	if event.Actor != "reviewer-23" {
		t.Fatalf("actor=%q, want reviewer-23", event.Actor)
	}
	audit := svc.AuditTrail(1)
	if len(audit) != 1 || audit[0].Actor != "reviewer-23" {
		t.Fatalf("audit actor=%q", audit[0].Actor)
	}
}
