...
func TestBug026AuditEqualTimestampOrder(t *testing.T) {
	svc := newService()
	svc.Ingest(app.IngestEnvelope{Kind: "order", Actor: "operator", Payload: "first-26"})
	svc.Ingest(app.IngestEnvelope{Kind: "order", Actor: "operator", Payload: "second-26"})
	events := svc.EventStream(0)
	if len(events) != 2 {
		t.Fatalf("events=%d", len(events))
	}
	if events[0].Payload != "second-26" || events[1].Payload != "first-26" {
		t.Fatalf("order=%q,%q", events[0].Payload, events[1].Payload)
	}
}
