...
// internal/domain/instrument_sessions.go
// internal/app/instrumentSession_service.go
// internal/httpapi/instrumentSession_handler.go
func TestBug005InstrumentsessionAdvanceLifecycle(t *testing.T) {
	svc := newService()
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	createdRec := postJSON(router, "/api/v1/instrument-sessions", `{"name":"sample-5","owner":"operator"}`)
	if createdRec.Code != http.StatusCreated {
		t.Fatalf("create status=%d body=%s", createdRec.Code, createdRec.Body.String())
	}
	var created domain.InstrumentSession
	decodeJSON(t, createdRec, &created)
	advancedRec := postJSON(router, "/api/v1/instrument-sessions/"+string(created.ID)+"/advance", `{"status":"active","actor":"reviewer"}`)
	if advancedRec.Code != http.StatusOK {
		t.Fatalf("advance status=%d body=%s", advancedRec.Code, advancedRec.Body.String())
	}
	var advanced domain.InstrumentSession
...
func TestBug005InvalidTransition(t *testing.T) {
	svc := newService()
	created, err := svc.CreateInstrumentSession(domain.InstrumentSession{Name: "sample-5", Owner: "operator"})
	if err != nil {
		t.Fatal(err)
	}
	_, err = svc.AdvanceInstrumentSession(created.ID, domain.StatusCompleted, "reviewer")
