...
func TestBug021AuthorizationErrorPropagation(t *testing.T) {
	svc := newService()
	_, err := svc.Apply(app.Command{Action: "approve", Actor: "", Subject: "subject-21"})
	if !errors.Is(err, app.ErrUnauthorized) {
		t.Fatalf("err=%v, want unauthorized propagation", err)
	}
	if len(svc.EventStream(0)) != 0 {
		t.Fatal("unauthorized command emitted an event")
	}
}

func TestBug021HealthyAuthorizedCommand(t *testing.T) {
	svc := newService()
