...
// internal/domain/policy_rules.go
// internal/app/policyRule_service.go
// internal/httpapi/policyRule_handler.go
func TestBug010PolicyruleAdvanceLifecycle(t *testing.T) {
	svc := newService()
	router := httpapi.NewRouter(svc, platform.NewLogger()).Handler()
	createdRec := postJSON(router, "/api/v1/policy-rules", `{"name":"sample-10","owner":"operator"}`)
	if createdRec.Code != http.StatusCreated {
		t.Fatalf("create status=%d body=%s", createdRec.Code, createdRec.Body.String())
	}
	var created domain.PolicyRule
	decodeJSON(t, createdRec, &created)
	advancedRec := postJSON(router, "/api/v1/policy-rules/"+string(created.ID)+"/advance", `{"status":"active","actor":"reviewer"}`)
	if advancedRec.Code != http.StatusOK {
		t.Fatalf("advance status=%d body=%s", advancedRec.Code, advancedRec.Body.String())
	}
	var advanced domain.PolicyRule
...
func TestBug010InvalidTransition(t *testing.T) {
	svc := newService()
	created, err := svc.CreatePolicyRule(domain.PolicyRule{Name: "sample-10", Owner: "operator"})
	if err != nil {
		t.Fatal(err)
	}
	_, err = svc.AdvancePolicyRule(created.ID, domain.StatusCompleted, "reviewer")
