...
type LineageNode struct {
	...
	InputIDs   []ID              `json:"inputIDs"`
	OutputID   ID                `json:"outputID"`
}
...
func (value LineageNode) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *LineageNode) Prepare(now time.Time) {
...
func (value *LineageNode) Advance(next Status, now time.Time) error {
...
func (value LineageNode) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "lineage-nodes",
		Name:      value.Name,
