...
func (value Protocol) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *Protocol) Prepare(now time.Time) {
...
func (value *Protocol) Advance(next Status, now time.Time) error {
...
func (value Protocol) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "protocols",
		Name:      value.Name,
