...
type ExperimentRun struct {
	...
	ProtocolID ID                `json:"protocolID"`
	SampleID   ID                `json:"sampleID"`
}

func (value ExperimentRun) Validate() error {
...
func (value ExperimentRun) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *ExperimentRun) Prepare(now time.Time) {
...
func (value *ExperimentRun) Advance(next Status, now time.Time) error {
...
func (value ExperimentRun) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "experiment-runs",
		Name:      value.Name,
		Status:    value.Status,
		Owner:     value.Owner,
