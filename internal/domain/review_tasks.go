...
func (value ReviewTask) Validate() error {
...
func (value ReviewTask) Key() string {
	return string(value.ID) + ":" + string(value.Status) + ":" + fmt.Sprint(value.Version)
}

func (value *ReviewTask) Prepare(now time.Time) {
...
func (value *ReviewTask) Advance(next Status, now time.Time) error {
...
func (value ReviewTask) View() EntityView {
	return EntityView{
		ID:        value.ID,
		Kind:      "review-tasks",
		Name:      value.Name,
