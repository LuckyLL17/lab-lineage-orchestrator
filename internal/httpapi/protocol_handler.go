...
func (r *Router) handleProtocols(w http.ResponseWriter, req *http.Request) {
	parts := pathParts(req, "/api/v1/protocols")
	if len(parts) == 0 {
		if req.Method == http.MethodGet {
			limit, _ := strconv.Atoi(req.URL.Query().Get("limit"))
			writeJSON(w, http.StatusOK, r.service.ListProtocols(req.URL.Query().Get("q"), limit))
			return
		}
		if req.Method == http.MethodPost {
			var input domain.Protocol
			if err := decodeJSON(req, &input); err != nil {
				writeError(w, http.StatusBadRequest, err)
				return
			}
			value, err := r.service.CreateProtocol(input)
			if err != nil {
	...
	if len(parts) == 1 && req.Method == http.MethodGet {
		value, ok := r.service.GetProtocol(id)
		if !ok {
	...
	if len(parts) == 2 && parts[1] == "advance" && req.Method == http.MethodPost {
		...
		value, err := r.service.AdvanceProtocol(id, input.Status, input.Actor)
		if err != nil {
